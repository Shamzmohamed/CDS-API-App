...
st.title("Interactive CD Plot")
st.sidebar.header("Hourly Plot - Selected Lat Lon")
st.write(
    """Select any weather parameters and specify the date you want to visualize. 
    Then select a point on the map to visualize the timeseries plot for that point. Then click Submit"""
)
...
if submitted:
    ...
    if params:            
        ...
        with st.spinner("Fetching weather data..."):
            ...
            # Connection to API
            conn = st.connection("", type=ClimateDataStoreConnection)
            df = conn.query(data)
        ...
        with st.spinner("Plotting..."):
            ...
            fig = px.line(df, x=df.index, y=params, labels={'x': 'Time', 'y': 'Parameter'})
            fig.update_layout(title="Time series plot of selected weather parameters",
