...
def get_total_precipitation(latitude, longitude):
    data = {
    ...
    # Check if the total precipitation column exists in the DataFrame
    if "total_precipitation" in df.columns:
        # Extract the total precipitation value
        total_precipitation = df['total_precipitation'].values[0]
    else:
        # The total precipitation column does not exist
...
with st.form("data_fetch_api"):
 ...
 #       selected_longitude = f_map["last_clicked"]["lng"]
    ...
    if f_map.get("last_clicked"):
        selected_latitude = f_map["last_clicked"]["lat"]
        selected_longitude = f_map["last_clicked"]["lng"]
        total_precipitation = get_total_precipitation(selected_latitude, selected_longitude)
        
        marker = folium.Marker(
        location=[selected_latitude, selected_longitude],
        tooltip=f"Total Precipitation: {total_precipitation} mm")
        marker.add_to(m)
